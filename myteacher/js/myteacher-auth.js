// ============================================================
// QAcademy Nurses Hub — MyTeacher Auth Utilities
// myteacher/js/myteacher-auth.js
//
// Mirror of js/auth.js — differences:
//   - createLoginSession writes to teacher_sessions (not sessions)
//   - Uses mt_session_id in localStorage (not qa_session_id)
//   - auth event and reset request writes target teacher_auth_events
//     and teacher_reset_requests via RPCs (same RPC pattern as auth.js)
//
// Shared helpers for authentication: hashing, device
// fingerprinting, and event ID generation.
// Used by myteacher/login.html and MyTeacher auth pages.
// ============================================================
...
/**
 * Creates a session row in teacher_sessions and stores the
 * session_id in localStorage under mt_session_id. This is the
...
async function createLoginSession(userId, loginVia) {
  ...
  const { error } = await db.from('teacher_sessions').insert({
