# MyNMCLicensure Sessions — 2026-04 to 2026-06 (the gamma era)

The pre-rebuild session log, carried over verbatim from the
`qacademy-gamma` repo on 2026-09-10 when the record files were
rebuilt to the one-line-index shape. Newest first. Entries describe the
vanilla-JS product that now lives in `legacy/`.
