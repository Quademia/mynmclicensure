...
| S7 | Attempt and offline-pack shape | Both keep an id list and re-read the live bank: `attempts.item_ids` comma-joined TEXT + `answers_json` a JSON string carrying a second copy of `correct`; `offline_packs.item_ids` a TEXT[] the renderer follows back on every open. The runner receives every column of every question | Split each into a header row plus one row per question, the question snapshotted as served (`attempt_items`, `offline_pack_items` — one set of snapshot machinery, two tables). The runner's projection sealed while live, unsealed in review, with a per-item unseal for instant feedback; student INSERT/UPDATE on attempts dropped for server-side writes. Supersedes S3. `quizzes` / `mock_quizzes` deliberately unchanged — a product on offer must follow the live bank. Adopted from MyNclex (Pillar 2 + snapshot tables), keeping `user_has_course()` and this product's three question types. Cheapest before cutover: D5 leaves **both** tables empty on launch day. Detail: `post-rebuild-diagnosis.md`, D5–D7, D12 and *Proposed direction — the attempts restructure* | ✅ Sam, 2026-09-18 · sliced 2026-09-20 as `03-quiz-system.md` Q4–Q6: the snapshot and the answer one row (`attempt_items`, no separate answers table), `started_utc` / `ended_utc` on the header, grading in SQL, the secret half revoked from the browser role, SATA partial credit after (Sam, 2026-09-20) |
...
| S14 | The premium marker on products | Premium is not stored. `/premium-prep` finds its products by parsing the id for the suffix `_2026_PREP` and taking the cheapest per programme (`premium-prep-client.tsx`, `PREP_SUFFIX`). Two faults: the suffix carries a **year**, so the convention expires in 2027 — every premium product renamed, or the code given a second suffix; and it is **weaker** than the rule the other two doors use, having no price gate at all, so a zero-price `_2026_PREP` product is selectable and payable where `paidProductsOf()` would refuse it | `products.is_premium boolean not null default false`, backfilled true for the five `_2026_PREP` rows; `/premium-prep` reads the flag and gains the missing price gate; the admin Products form gains the toggle; `PREP_SUFFIX` goes. **No tier column** — `kind` already carries TRIAL / FREE / PAID (6 / 5 / 21 active on dev) and premium is a subset of PAID, so a boolean is the whole of what is missing. **Rule 9 / D43:** `products` still held the vanilla-era `grant all` for both browser roles (DELETE, INSERT, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE), so this slice takes it back to SELECT — `anon` keeps SELECT, unlike `course_access`, because the public pages read the catalogue through the cookie client. The admin writes move to the service role behind `requireAdmin()`, and `products_insert` / `products_update` are dropped, having policed a privilege no role holds. Which **programme** a premium product belongs to stays id-derived: D23 item 2 rules the real answer (`courses.program_scope`), which needs data this page does not load and belongs to the shop slice | ✅ Sam, 2026-09-22, option A (the full rule-9 job, not reads only). Ruled earlier as D23 item 4, 2026-09-18; this row is the storage authorisation |
| S15 | One subscription per Paystack payment | `activatePaymentForUser()` checks for a subscription with this reference, then inserts one — check-then-write with no constraint behind it, so two callers in the same second both insert and the buyer gets two receipts (two years queued). Latent today (two tabs on the confirmation page); routine once a webhook and the returning browser race. `subscriptions` also still holds the vanilla-era `grant all` for both browser roles, and the admin page writes through the cookie client on `subscriptions_insert` / `subscriptions_update` | A unique index on `subscriptions (source_ref) where source = 'PAYSTACK'`, the migration refusing to run if a duplicate exists; activation treats the unique refusal as "the other door won" and re-reads that receipt. **Rule 9 / D43:** revoke all from `anon` and `authenticated`, grant `authenticated` SELECT back; the admin writes (`lib/subscriptions/actions.ts`) move to the service role behind `requireAdmin()`; `subscriptions_insert` / `subscriptions_update` dropped. Dev: 1 PAYSTACK row, 1 distinct reference. The prerequisite of the D4 webhook | ✅ Sam, 2026-09-23, as drafted (the rule-9 job included) |
| S16 | The free pool's door (09 F1) | Every read of `question_bank` and every attempt creation asks one question — does this student hold the course (`user_has_course()`) — so a student with no `course_access` row reads nothing and can start nothing | A second question beside the first, in both places: the read policy also opens a row whose `is_free_sample` is true to any signed-in student, and `create_attempt` also accepts a set of item ids when every one of them is marked free. The mark is the column S17 adds; this row is the door, because it changes the security floor. The free rows stay inside their paid course (a paid student meets them as normal); a row named by any mock can never be marked free. Where the pool lives was 09 §4's open question, answered as a mark, not a separate table (Sam, 2026-09-26): the same rows, importer, editor and snapshot serve both, and a second table would duplicate all of them and every column S17 adds; the pool needs a size per programme and a refresh rule for the daily challenge, which is content, not storage | adopted in session 2026-09-26 (Sam, items 3 and 4 of the MyNclex bank comparison, 08 §3) — ✅ Sam, 2026-09-26 |
| S17 | The bank's new columns, the version and the history | `question_bank` has 26 columns and two constraints (the key and the course key): no draft state (a saved or imported row is live at once), no free mark, no level, no source, no tags, no dates, no version, no history; `question_type` and `difficulty` are unchecked text, so a file's "T/F" lands as a type no code knows and the runner cannot render | Nine columns (eight taken from MyNclex's bank after a column-by-column comparison, plus `updated_by text`, server-only — the admin's id on the row, because the history trigger cannot see the actor under the service role; detail refined at the scoping the same day): `bloom_level` (six levels; the count, the spelling and student visibility open), `is_published` (default false for a new row; the 5,281 existing rows published at migration; every student-side read adds the condition; the importer offers publish now or drafts), `is_free_sample` (default false; the door is S16), `question_ref` (internal provenance, free text with commas, never shown to a student, never copied), `tags` (text[], free text with suggestions from tags in use, matched without case; student-facing; a Tags panel to rename, merge and delete), `created_at` / `updated_at` (existing rows take the migration's date), `version` (integer from 1). A history table `question_bank_history` — the bank's columns plus `version`, `changed_by`, `changed_at`, `deleted` — written by one trigger with the row as it was before a **content** change to a **published** row (stem, options, correct, rationale, feedbacks, type, marks) and on delete; drafts and label-only changes (subject, topic, subtopic, level, tags, source, difficulty, the two switches) write no history; restore is a save, the chosen version's content written onto the row and the current one pushed to history. CHECKs on `question_type` (MCQ, TF, SATA), `difficulty` (Easy, Moderate, Hard) and `bloom_level` (the six, or null). `attempt_items` and `offline_pack_items` gain `bloom_level`, `tags` and `version`, copied at creation as the four classification fields are — names, not keys; no date, no source, no switches. Rule 9 at the scoping: the history table gets `revoke all` from both browser roles (a new table here is born with `grant all`); `authenticated` gains SELECT on `is_published` and `is_free_sample` only, `anon` nothing; `question_ref` and `updated_by` never; the search function and both copiers refuse a draft — the spec is 08 B4 | adopted in session 2026-09-26 (items 1, 2, 5, 6, 6b and 8 of the comparison, 08 §3; the choice at save between a new version and an overwrite, Sam's own idea, set aside for the automatic rule) — ✅ Sam, 2026-09-26 · built 2026-09-26 as 08 B4 (`20260926150000_question_bank_columns_history.sql`, `20260926160000_copiers_refuse_any_draft.sql`) |
| S18 | The classification lists | `subject`, `maintopic` and `subtopic` are free text on every row: General Paper's subject holds 149 values, the two RMHN courses have no topic on any of their 720 rows, NACNAP Clinical has 148 topics across 180 questions, and the builder's topic list is built from these words | Two lookup tables per course, `bank_subjects` and `bank_topics` (course, name, retired), with keys from the bank rows' subject and topic to them; subtopic stays free text. The editor's dropdowns read the tables; the importer refuses a row naming an unknown subject or topic with a message naming the word, and never creates one; a Topics panel on the Courses page with add, rename (flows to every question), retire (hidden from the dropdowns, questions untouched), delete refused while in use. Subject means the academic module a question draws on (the licensure paper is the course; a student's training was many modules), stays per course (pharmacology in Medicine is not pharmacology in Surgery — Sam), and carries no programme column (the course already knows its programme, and General Paper's is five). The eight courses whose subject holds the course's own name count as unlabelled. The one-time clean-up per course is content work with the writers, from a first-draft list an assistant builds from the words already there, for Sam to correct. **Amended by Sam the same day (B5 talked through, 08 §3):** the lists are seeded from the words already there and no question's words change, except the course-name subjects cleared to Not set; an empty subject or topic is Not set (null), with no built-in "Other"; one topic per question, a second area going in tags; the importer accepts a blank cell; the panel is **one combined Subjects & topics panel on the Question Bank page** (not a Topics panel on the Courses page), for both lists, with merge added and, on topics, split into topic + tag; the clean-up is done in that panel | adopted in session 2026-09-26 (item 7 of the comparison, 08 §3) — ✅ Sam, 2026-09-26; amended 2026-09-26 · built 2026-09-26 as 08 B5 (`20260926170000_bank_subject_topic_lists.sql`) |
| S19 | A student's read of the bank | Since 08 B2 `authenticated` holds column-level SELECT on 19 columns of `question_bank`, the RLS policy scoping the rows to the student's published course rows. The app's student-side reads use 10 of them — `knownItemIds`, `getBuilderCourseItems` and `getItemFilterOptions` read the id, course, subject, topic, subtopic, difficulty, type, batch and the two switches (the admin page's filter options read the batch through the admin's own cookie client, which is `authenticated` too). The other 9 — `stem`, `option_a`…`option_f`, `marks`, `shuffle_options` — no code reads through a browser client: a sitting's and a pack's questions are copied by `create_attempt` / `create_offline_pack` (definer, service role) and read through the service role. So a subscribed student's own login can read the wording and options of every published question in their courses straight from the API — **a mock's questions before it is sat**, which 08 B6 holds back from practice, and a whole course's question text to copy out | `revoke select (stem, option_a, option_b, option_c, option_d, option_e, option_f, marks, shuffle_options) on question_bank from authenticated`; the 10 filter columns stay; `anon` holds nothing since B2. Question text then reaches a student only inside a sitting or a pack. No screen changes; no function or policy reads the 9 as `authenticated` (checked 2026-09-26: the four functions reading the bank are definer or run by the service role). Proven in a rolled-back run under `set role authenticated` before the apply; `role_column_grants` checked after. Built with 08 B6 | adopted in session 2026-09-26 (Sam, B6 talked through: "3 accepted") — ✅ Sam, 2026-09-26 · built 2026-09-26 with 08 B6 (`20260926180000_question_bank_student_read_narrowed.sql`) |
...
| 22 | The confirmation page verifies every 3 seconds, up to 20 times, while Paystack has not said success — and the Worker's rate limit allows 5 calls per 60 seconds per address on the same route. So a payer whose mobile-money payment is still pending after about twelve seconds sees "Too many requests" and must tap Retry Verification, instead of the "still verifying" screen the poll was written for | `payment-confirmation.html` POLL_MS / MAX_POLLS, `payment-worker` wrangler ratelimits | Seen in the slice 9a walk (2026-09-15) on an unpaid reference: four polls, then the sixth call refused. Both numbers transcribed as they were. **Carried** (Sam, 2026-09-15); listed under "After the rebuild" — the fix is either a slower poll or a limit that excludes verify. **Closed 2026-09-23 by D35:** verify counted per payment at 30 a minute, above the poll's pace, and a refused check shown as "still checking" |
