// app/(app)/runner/instant/page.tsx — legacy runner/instant.html.
//
// The practice-mode runner. The server half runs legacy's preflight
...
export const metadata: Metadata = {
  title: 'Practice Quiz | MyNMCLicensure',
};
...
export default async function InstantRunnerPage({ searchParams }: { searchParams: Promise<Params> }) {
  const gate = await requireStudent();
  const sp = await searchParams;

  const load = await loadRunner(gate, 'instant', {
